...
const main = () => {
  ...
  optionStart.innerText = `Choose`;
  optionStart.classList.add("start");
  select.appendChild(optionStart);
  users.forEach((user) => {
    select.appendChild(creatSet(user));
  });
  document.querySelector("#form").addEventListener("change", getIdUser);
};
...
const getIdUser = (event) => {
  ...
  const idUser = +event.target.value.split("_")[1];
...
const renderUser = (index) => {
  const divUser = document.querySelector("#user-render");
  divUser.innerHTML = "";
  if (index === -1) {
  ...
  div.appendChild(createElementUser("h3", "Name", users[index].name));
  div.appendChild(createElementUser("h5", "Nickname", users[index].username));
  div.appendChild(createElementUser("p", "Email", users[index].email));
  div.classList.add("user");
  divUser.appendChild(div);
};

const createElementUser = (tagName, str, property) => {
  const tag = document.createElement(`${tagName}`);
  tag.innerText = `${str} ${property}`;
  return tag;
};
const createIdElement = (name, id) => {
  return `${name}${id}`;
};

const renderAlbum = (idUser) => {
  const albumsUser = albums.filter((album) => album.userId === idUser);
  const divAlbums = document.querySelector("#albums");
  divAlbums.innerHTML = "";
  albumsUser.forEach((album) => {
    const tag = createElementUser("h4", "Album", album.title, "album");
    const divAlbum = createElementUser("div", "", "");
    const divAlbumWra=createElementUser("div","","");
    divAlbum.id = createIdElement(`divalbumphotos_`, album.id);
    tag.id = createIdElement(`album_`, album.id);
    divAlbumWra.id =createIdElement("divalbum_", album.id);
    tag.classList.add("album")
    divAlbumWra.classList.add("photos");
    divAlbum.classList.add("hiden");
    divAlbum.appendChild(divAlbumWra);
    divAlbums.appendChild(tag);
    divAlbums.appendChild(divAlbum);
    tag.addEventListener("click", getIdAlbum);
    renderPhotos();
  });
};

const getIdAlbum = (event) => {
  event.preventDefault();
  const idAlbum = event.target.id.split("_")[1];
  const idDivAlbum=`divalbumphotos_${idAlbum}`
  changeClass(idDivAlbum);
};

const changeClass=(id)=>{
document.querySelector(`#${id}`).classList.toggle("hiden")
renderPhotos();
}

const renderPhotos = () => {
  const divAlbums = document.querySelectorAll(".photos");
  for (divAlbum of divAlbums) {
    let id = +divAlbum.id.split("_")[1];
    const photosAlbum = photos.filter((photo) => photo.albumId === id);
    photosAlbum.forEach((photo) => {
      const div = document.createElement("div");
      div.id = createIdElement("photo_", photo.id);
      div.addEventListener("click", renderPhotoBig);
      div.style.backgroundImage = `url(${photo.thumbnailUrl})`;
      divAlbum.appendChild(div);
    });
  }
};
const renderPhotoBig =(event)=>{
const id =+event.target.id.split("_")[1];
window.open(photos.filter(photo=>photo.id===id)[0].url);
}
main();
